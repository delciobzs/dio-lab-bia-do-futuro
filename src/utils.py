...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(ARQUIVO_SISTEMA, encoding="utf-8"),
        logging.StreamHandler()
    ]
)

def obter_logger(nome_modulo: str):
    """Retorna um logger nomeado para identificar a origem dos eventos."""
    return logging.getLogger(nome_modulo)

logger_ferramentas = obter_logger("ferramentas")
...
def obter_estilo_usuario(caminho_dados: Path) -> tuple:
    ...
    try:
        ...
        logger_ferramentas.error(f"Erro ao calcular estilo por idade: {erro}", exc_info=True)
        return "Direto e profissional", "Didático,mas sem enrolação"
...
def buscar_historico_atendimento(tema: str = "") -> str:
    """
        ...
        """
    logger_ferramentas.info(f"Ferramenta acionada: buscar_historico_atendimento | Parâmetro: tema='{tema}'")
    ...
    try:
        ...
        if df_filtrado.empty:
            logger_ferramentas.warning(f"Nenhum ticket encontrado para o tema '{tema}'.")
...
def consultar_ultimas_transacoes() -> str:
    ...
    try:
        ...
        logger_ferramentas.error(f"Erro ao processar transacoes.csv: {erro}", exc_info=True)
        return "Erro interno ao consultar as transações."
...
def consultar_produtos_financeiros(risco: str) -> str: # receberá risco ou perfil do investidor
    ...
    logger_ferramentas.info(f"Ferramenta acionada: consultar_produtos_financeiros | Parâmetro: risco='{risco}'")
    ...
    try:
        ...
        logger_ferramentas.error(f"Erro inesperado ao processar produtos_financeiros.json: {erro}", exc_info=True)
...
def atualizar_carteira_investimentos(ativo: str, valor: float, operacao: str) -> str:
    ...
    """
        ...
        """
    logger_ferramentas.info(f"Ferramenta: atualizar_carteira | {operacao} {ativo} - R$ {valor:.1f}")
    caminho_arquivo = CAMINHO_DADOS / "perfil_investidor.json"
    ...
    try:
        ...
        logger_ferramentas.error(f"Erro ao atualizar carteira: {erro}", exc_info=True)
        return "Erro interno ao atualizar a carteira de investimentos no JSON."
...
def registrar_nova_transacao(descricao: str, categoria: str, valor: float, tipo: str) -> str:
    """
        ...
        """
    logger_ferramentas.info(f"Ferramenta: registrar_transacao | {descricao} - R$ {valor:.2f}")
    caminho_arquivo = CAMINHO_DADOS / "transacoes.csv"
    ...
    try:
        ...
        logger_ferramentas.error(f"Erro ao registrar transação: {erro}", exc_info=True)
        return "Erro interno ao registrar a movimentação financeira."
...
def gerar_relatorio_financeiro() -> str:
    ...
    """
    logger_ferramentas.info(f"Ferramenta acionada: gerar_relatorio_financeiro")
